...
@export var explosion_collision_shape: CollisionShape2D

var damageable_objects: Array = []


func _ready() -> void:
	explosion_collision_shape.shape.radius = Parameters.ICE_EXPLOSION_RADIUS


func explode():
	ice_particles.emitting = true
	ice_particles.emission_sphere_radius = 0.0
	create_tween().tween_property(ice_particles, "emission_sphere_radius", 128.0, ice_particles.lifetime * Parameters.ICE_EXPANDING_SPEED_MULTIPLIER)
